...
def format_metrics_html(metrics_data: dict = None) -> str:
    """Format performance metrics as styled HTML table"""
    ...
    if metrics_data:
        asr = metrics_data.get('asr_processing_time')
        llm = metrics_data.get('first_llm_token_time')
        tts = metrics_data.get('first_tts_chunk_time')
        total = metrics_data.get('total_processing_time')
        
        if asr is not None: asr_time = f"{asr:.2f}s"
        if llm is not None: llm_time = f"{llm:.2f}s"
        else: llm_time = "N/A"
        if tts is not None: tts_time = f"{tts:.2f}s"
        if total is not None: total_time = f"{total:.2f}s"
    ...
    <div id="metrics-content" style="background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.05); font-family: sans-serif; border: 1px solid #e9ecef;">
        ...
        <div style="padding: 0 16px;">
            <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f1f3f5;">
                <span style="color: #868e96; font-size: 13px;">Processing</span>
                <span style="color: #212529; font-weight: 600; font-size: 14px;">{asr_time}</span>
            </div>
            <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f1f3f5;">
                <span style="color: #868e96; font-size: 13px;">LLM First Token</span>
                <span style="color: #212529; font-weight: 600; font-size: 14px;">{llm_time}</span>
            </div>
            <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #f1f3f5;">
                <span style="color: #868e96; font-size: 13px;">TTS First Chunk</span>
                <span style="color: #212529; font-weight: 600; font-size: 14px;">{tts_time}</span>
            </div>
            <div style="display: flex; justify-content: space-between; padding: 10px 0;">
                <span style="color: #495057; font-size: 13px; font-weight: 600;">Total Time</span>
                <span style="color: #1971c2; font-weight: 700; font-size: 14px;">{total_time}</span>
            </div>
        </div>
    </div>
    """
...
# TTS Languages with display names and response instructions
TTS_LANGUAGES = {
    "en": {
...
# NOTE: Hebrew input is supported, but TTS output in Hebrew is NOT available
# When using Hebrew input, the agent will understand but respond in selected TTS language
ASR_LANGUAGES = {
    "auto": "🌐 Auto-detect",
    "en": "🇺🇸 English",
...
async def process_audio(
    ...
    try:
        ...
        status_log += f"\n✅ Creating audio file from {len(audio_buffer):,} bytes...\n"
        ...
        # Save to file instead of base64 to avoid protocol errors with large payloads
        import uuid
        
        try:
            # Create temp file
            filename = f"response_{uuid.uuid4().hex}.wav"
            # Use Gradio's temp dir structure or system temp
            temp_dir = Path(tempfile.gettempdir()) / "gradio"
            temp_dir.mkdir(exist_ok=True)
            ...
            status_log += f"📁 Saved to: {filename}\n"
            status_log += "=" * 50 + "\n"
            
            print(f"[UI] Audio saved to: {filepath}")
            ...
            yield filepath, status_log, gr.update(value=None), gr.update(), final_metrics_update, gr.update()
            
        except Exception as save_err:
            print(f"[UI] Error saving audio: {save_err}")
            import traceback
            traceback.print_exc()
            status_log += f"❌ Error: {save_err}\n"
            yield gr.update(), status_log, gr.update(), gr.update(), gr.update(value=last_perf_html, visible=True), gr.update()
...
def create_ui():
    ...
    # =========================================================================
    # MODERN UI & CSS - Professional Design System (v5.0)
    # =========================================================================
    custom_css = """
    /* --- FONTS & BASICS --- */
    ...
    with gr.Blocks(title="HPE AI Voice Agent v4.1.0", theme=modern_theme, css=custom_css) as demo:
        ...
        **Interaction Modes:**
        ...
        🌍 **Supported Languages:** English, Hebrew, Spanish, French, German, Italian, Portuguese, Russian, Chinese, Japanese, Korean, Arabic and 90+ more!
        """)
        ...
        💡 Tip: Press <b>Spacebar</b> to toggle recording
                    ...
                    with gr.Column(scale=1):
                        ...
                        <div id="audio-player-container" style="padding: 10px; background: #f0f4f8; border-radius: 8px; margin-bottom: 10px; position: relative;">
                            ...
                            <audio id="custom-audio-player" controls style="width: 100%;" autoplay playsinline crossorigin="anonymous">
            ...
            with gr.TabItem("🔊 Voice Output"):
                ...
                gr.Markdown("*XTTS v2 supports 16 languages with natural voice synthesis.*")
                ...
                with gr.Tabs():
                    with gr.TabItem("Settings"):
                        ...
                        # =====================================================
                        # XTTS v2 BUILT-IN SPEAKERS (VERIFIED GENDER + ACCENT)
                        # Format: "Name" - Accent/Style note
                        # =====================================================
                        
                        # FEMALE VOICES - Verified female speakers
                        # FEMALE VOICES - Verified female speakers with accent info
                        XTTS_VOICES_FEMALE = [
                            # American English Female
                            "Daisy Studious",      # 🇺🇸 American - Clear, professional
                            "Gracie Wise",         # 🇺🇸 American - Warm, friendly
                            "Nova Hogarth",        # 🇺🇸 American - Modern, clear
                            "Brenda Stern",        # 🇺🇸 American - Authoritative
                            "Tammy Grit",          # 🇺🇸 American - Casual, natural
                            # British/Irish Female
                            "Claribel Dervla",     # 🇮🇪 Irish - Soft, melodic
                            ...
                            # German Female
                            "Henriette Usha",      # 🇩🇪 German - Clear
                            "Gitta Nikolina",      # 🇩🇪 German - Warm
                            "Tanja Adelina",       # 🇩🇪 German - Professional
                            # French Female
                            "Annmarie Nele",       # 🇫🇷 French - Elegant
                            # Nordic Female
                            "Camilla Holmström",   # 🇸🇪 Swedish - Clear
                            "Maja Ruoho",          # 🇫🇮 Finnish - Soft
                            # Eastern European Female
                            "Szofi Granger",       # 🇭🇺 Hungarian - Melodic
                            "Zofija Kendrick",     # 🇵🇱 Polish - Clear
                            "Barbora MacLean",     # 🇨🇿 Czech - Professional
                            "Lidiya Szekeres",     # 🇷🇺 Russian - Warm
                            "Lilya Stainthorpe",   # 🇷🇺 Russian - Soft
                            # Spanish/Portuguese Female
                            "Alma María",          # 🇪🇸 Spanish - Warm
                            "Roziya Estella",      # 🇵🇹 Portuguese - Warm
                            # Italian Female
                            "Vjollca Johnnie",     # 🇮🇹 Italian - Melodic
                            # Middle Eastern Female
                            "Asya Anara",          # 🇹🇷 Turkish - Soft
                            # Asian Female
                            "Alexandra Hisakawa",  # 🇯🇵 Japanese - Clear
                            "Chandra MacFarland",  # 🇮🇳 Indian - Professional
                            "Narelle Moon",        # 🇦🇺 Australian - Friendly
                            # African Female
                            "Rosemary Okafor",     # 🇳🇬 Nigerian - Warm
                            "Ige Behringer",       # 🌍 African - Clear
                            # Latin American Female
                            "Uta Obando",          # 🌎 Latin American - Friendly
                        ]
                        
                        # MALE VOICES - Verified male speakers with accent info
                        XTTS_VOICES_MALE = [
                            ...
                            "Zacharie Aimilios",   # 🇬🇧 British - Clear
                            # German Male
                            "Baldur Sanjin",       # 🇩🇪 German - Strong
                            "Aaron Dreschner",     # 🇩🇪 German - Professional
                            "Wulf Carlevaro",      # 🇩🇪 German - Clear
                            # French Male
                            ...
                            "Luis Moray",          # 🇪🇸 Spanish - Friendly
                            # Portuguese Male
                            "Marcos Rudaski",      # 🇧🇷 Brazilian - Clear
                            # Eastern European Male
                            "Damjan Chapman",      # 🇷🇸 Slavic - Clear
                            "Ludvig Milivoj",      # 🇸🇪 Swedish - Professional
                            "Anton Khorev",        # 🇷🇺 Russian - Deep
                            "Ilkin Urbansen",      # 🇺🇦 Ukrainian - Clear
                            # Mediterranean Male
                            "Viktor Menelaos",     # 🇬🇷 Greek - Warm
                            "Eugenio Mataracı",    # 🇹🇷 Turkish - Professional
                            "Arda Jung",           # 🇹🇷 Turkish - Professional
                            # Middle Eastern Male
                            "Suad Qasim",          # 🇸🇦 Arabic - Clear
                            "Badr Odhiambo",       # 🇸🇦 Arabic/African - Warm
                            # Asian Male
                            "Kazuhiko Atallah",    # 🇯🇵 Japanese - Professional
                            "Xavier Hayasaka",     # 🇯🇵 Japanese - Clear
                            "Kumar Dahl",          # 🇮🇳 Indian - Warm
                        ]
                        
                        ALL_XTTS_VOICES = ["default"] + XTTS_VOICES_FEMALE + XTTS_VOICES_MALE
                        
                        # Recommended voices per language (based on accent/nationality)
                        VOICES_BY_LANG = {
                            "en": {
                            ...
                            "fr": {
                                "female": ["Annmarie Nele", "Camilla Holmström", "Gitta Nikolina"],
                                "male": ["Adde Michal", "Filip Traverse", "Viktor Eka"]
                            },
                            "de": {
                                "female": ["Henriette Usha", "Gitta Nikolina", "Tanja Adelina", "Szofi Granger"],
                                "male": ["Baldur Sanjin", "Aaron Dreschner", "Wulf Carlevaro", "Viktor Eka"]
                            },
                            "it": {
                                "female": ["Vjollca Johnnie", "Sofia Hellen", "Lidiya Szekeres"],
                                "male": ["Gilberto Mathias", "Viktor Menelaos", "Eugenio Mataracı"]
                            },
                            "pt": {
                                "female": ["Roziya Estella", "Alma María", "Narelle Moon"],
                                "male": ["Marcos Rudaski", "Luis Moray", "Dionisio Schuyler"]
                            },
                            "pl": {
                                "female": ["Zofija Kendrick", "Barbora MacLean", "Lidiya Szekeres"],
                                "male": ["Damjan Chapman", "Ludvig Milivoj", "Anton Khorev"]
                            },
                            "tr": {
                                "female": ["Asya Anara", "Ige Behringer"],
                                "male": ["Eugenio Mataracı", "Arda Jung", "Suad Qasim"]
                            },
                            "ru": {
                                "female": ["Lidiya Szekeres", "Lilya Stainthorpe", "Barbora MacLean"],
                                "male": ["Anton Khorev", "Ilkin Urbansen", "Viktor Menelaos", "Damjan Chapman"]
                            },
                            "nl": {
                                "female": ["Camilla Holmström", "Tammie Ema", "Alison Dietlinde"],
                                "male": ["Damjan Chapman", "Zacharie Aimilios", "Ludvig Milivoj"]
                            },
                            "cs": {
                                "female": ["Barbora MacLean", "Zofija Kendrick", "Szofi Granger"],
                                "male": ["Damjan Chapman", "Filip Traverse", "Ilkin Urbansen"]
                            },
                            "ar": {
                                "female": ["Asya Anara", "Ige Behringer", "Rosemary Okafor"],
                                "male": ["Badr Odhiambo", "Suad Qasim", "Kumar Dahl"]
                            },
                            "zh-cn": {
                                "female": ["Alexandra Hisakawa", "Chandra MacFarland", "Narelle Moon"],
                                "male": ["Kazuhiko Atallah", "Xavier Hayasaka", "Kumar Dahl"]
                            },
                            "ja": {
                                "female": ["Alexandra Hisakawa", "Narelle Moon", "Chandra MacFarland"],
                                "male": ["Kazuhiko Atallah", "Xavier Hayasaka", "Viktor Eka"]
                            },
                            "hu": {
                                "female": ["Szofi Granger", "Lidiya Szekeres", "Barbora MacLean"],
                                "male": ["Ludvig Milivoj", "Damjan Chapman", "Viktor Menelaos"]
                            },
                            "ko": {
                                "female": ["Alexandra Hisakawa", "Chandra MacFarland", "Narelle Moon"],
                                "male": ["Kazuhiko Atallah", "Xavier Hayasaka", "Kumar Dahl"]
                        ...
                        tts_voice = gr.Dropdown(
                            ...
                            info="Select a voice for the agent"
                        )
            ...
            with gr.TabItem("📊 Data Viewer"):
                ...
                with gr.Tabs():
                    ...
                    with gr.TabItem("😠 Sentiment Alerts"):
                        ...
                        gr.Markdown("""
                        **Sentiment alerts are triggered when customers express:**
                        - 🔴 **High Risk:** Cancellation intent, very negative sentiment
                        - 🟡 **Medium Risk:** Frustration, complaints
            ...
            with gr.TabItem("ℹ️ About"):
                ...
                *   **Natural Output:** High-quality neural text-to-speech (XTTS v2) in 16 languages.
                ...
                *   **Sentiment Analysis:** Detects customer frustration and churn risk automatically.
                ...
                #### 3. Advanced UI/UX
                *   **Live Visualization:** Real-time audio waveform and frequency analysis.
            ...
            function monitorAudioDataHolder() {
                ...
                const checkForAudioData = () => {
                    ...
                    if (holder && holder.value && holder.value.length > 5) {
                        let audioUrl = holder.value;
                        
                        // Add /file= prefix if it's a direct path and not already formatted
